...
小財是浮在 Android 桌面上的財務桌寵，點牠一下就能記一筆帳 —— App 沒開也可以。
網站版是同一本帳的完整版：明細、預算、負債、目標、長期試算表、AI 金流推論都在。

> 專案倉庫與部分文件仍使用舊名 FinTracker（Android package id `com.fintracker.app`
> 也刻意沒有改 —— 改了會讓已安裝的使用者無法更新）。使用者看到的名字是「小財記帳」。
...
| 分頁 | 用途 |
|---|---|
| 🏠 首頁 | 今天花多少、這個月花多少、預算還剩多少、最近五筆。不用捲動 |
| 🧾 明細 | 全部紀錄、搜尋、過濾、CSV 匯出 |
| 🐣 小財 | 開關桌寵、快速記帳設定、資料匯出／匯入 |
| ⋯ 更多 | 預算與循環、負債與目標、長期試算表、AI 顧問、下載 App |

「更多」裡的功能一個都沒有被刪掉，只是不再佔用主導覽的位置。桌機版維持完整分頁列。
...
| | 進入點 JS | gzip |
|---|---|---|
| 之前（單一 bundle） | 1,236 kB | 358 kB |
| 現在（低頻功能 lazy） | 338 kB | 109 kB |

圖表（recharts）、AI 面板、試算表、目標沙盒都各自在自己的 chunk 裡，用到才下載。
...
| `npm run test` | 單元與回歸測試（vitest） |
| `npm run icons` | 重繪 Android launcher icon（所有密度） |
...
即使停用桌寵，App 本身在手機上也好用：底部導覽是 首頁／明細／小財／更多，
「記一筆」在首頁與小財頁都是整頁最大的按鈕。
...
打開就看得到的只有四樣：**大小**、**桌寵名稱**、**預設類型**、**快速模式**，
再加上**資料匯出／匯入**（備份是資料安全，不該藏起來）。

其餘全部收在「**進階設定**」裡，展開就有：停靠邊、自動收邊、動畫、透明度、
釘選分類、自訂分類、支付方式、泡泡顯示程度、App 鎖定、帳號與同步、權限與診斷。
...
- 網頁的 `lang` 必須是 `zh-Hant-TW` 且帶 `translate="no"`：手機瀏覽器的自動翻譯會改寫
  React 管理的 DOM 節點，造成 `removeChild` 崩潰。`src/__tests__/translateGuard.test.ts`
  會擋住這幾個標記被拿掉。
